...
file_path = f"./{dataset}/selected_configs.txt"
...
for rd_number in range(len(number_runs)):
    ...
    for config_number in range(configs_setups[rd_number]):
        if rd_number == 0:
            final_fitness = main(dataset,selected_configs[config_number][0], selected_configs[config_number][1], selected_configs[config_number][2], selected_configs[config_number][3], selected_configs[config_number][4], number_runs[rd_number], rd_number, config_number, 0)

            configs_fitness[config_number] = final_fitness
        else:
            sorted_config_number = sorted_indices[config_number]
            final_fitness = main(dataset, selected_configs[sorted_config_number][0], selected_configs[sorted_config_number][1], selected_configs[sorted_config_number][2], selected_configs[sorted_config_number][3], selected_configs[sorted_config_number][4], number_runs[rd_number], rd_number, sorted_config_number, 0)
   ...
   # Convert configs_fitness to a numpy array
    ...
    np.save("./{}/rd{}/sorted_indices".format(dataset, rd_number), sorted_indices)
